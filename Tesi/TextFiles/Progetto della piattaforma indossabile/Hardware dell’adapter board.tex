...
\begin{figure}[b]
	...
	c$)$
	\includegraphics[width=0.1567\linewidth]{ImageFiles/Hardware/manifacturing_top_maxm}
	d$)$
	\includegraphics[width=0.1567\linewidth]{ImageFiles/Hardware/manifacturing_bottom_maxm}
...
\paragraph{LDO} Per fornire l'alimentazione di 1.8V al modulo PPG è stato inserito il regolatore lineare di tensione (LDO), questo è un dispositivo che, data una tensione in ingresso, ne fornisce una costante in uscita di valore anche differenze da quello in ingresso. Nel caso dell'\textit{Adapter Board} realizzata, questo dispositivo viene utilizzato per alimentare l'accelerometro e la circuiteria interna del sensore PPG.
Nella scelto del regolatore lineare di tensione sono stati presi in considerazione tre diversi dispositivi:
\todo{tabella con i 3 LDO in stile cossali}


\textbf{ADP166}, prodotto da Analog Devices\cite{AnalogDevicesADP166}.\todo{descrizione LDO ADP166}
...
\todo{tabella tra i tre LDO considerati}
...
\paragraph{Progetto PCB} Anche il progetto di questa board è stato realizzato con il software Autodesk Eagle, secondo gli stessi passi seguiti per la precedente. In figura \ref{fig:schematic_max} viene riportato lo schematico dove si possono vedere il sensore PPG, il regolatore di tensione lineare(LDO), l'accelerometro e i due connettori per l'alimentazione della scheda.
\begin{figure}[b]
	\centering
...
\begin{figure}[tb]
	\centering
	a$)$
	\includegraphics[width=0.15\linewidth]{ImageFiles/Hardware/layout_top_max}
	b$)$
	\includegraphics[width=0.15\linewidth]{ImageFiles/Hardware/layout_bottom_max}
	c$)$
	\includegraphics[width=0.1568\linewidth]{ImageFiles/Hardware/manifacturing_top_max}
	d$)$
	\includegraphics[width=0.1568\linewidth]{ImageFiles/Hardware/manifacturing_bottom_max}
